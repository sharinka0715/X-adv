...
from layers import Detect, PriorBox, L2Norm, DOAM


class SSD_DOAM(nn.Module):
    ...
    def forward_conv(self, x, n_layer):
    ...
    def forward(self, x):
        ...
        if self.phase == "test":
            output = self.detect(
                ...
                self.priors.to(loc.device)                        # default boxes
            )
...
def build_ssd(phase, size=300, num_classes=21):
    ...
    if(phase == 'train'):
        ...
        base_, extras_, head_ = multibox(vgg(base[str(size)], 4),
                                         add_extras(extras[str(size)], 1024),
                                         mbox[str(size)], num_classes)
    from data.config import DOAM
    return SSD_DOAM(phase, size, base_, extras_, head_, num_classes, DOAM)
